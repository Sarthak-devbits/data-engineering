...
export async function syncAgedReceivablesByContact(
  token: string,
  tenantId: string,
  xeroUserId: string,
  userId: number,
): Promise<void> {
  logger.info("==> Syncing Aged Receivables By Contact...");
  ...
  logger.info(`Processing aged receivables for ${contacts.length} contacts...`);

  const today = new Date();
  const oneYearAgo = new Date(today);
  oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1);
  const fromDate = toDateString(oneYearAgo);
  ...
  for (const contact of contacts) {
    ...
    try {
      const data = await callApi<XeroReportResponse>(
        tenantId,
        token,
        `Reports/AgedReceivablesByContact?ContactID=${contactId}&fromDate=${fromDate}&toDate=${toDate}`,
      );
      ...
      for (const section of rows.slice(1)) {
        ...
        for (const subRow of section.Rows) {
          ...
          try {
            const existing = await prisma.agedReceivableByContact.findFirst({
              where: {
                number: rowData[1] || null,
            ...
            if (!existing) {
              await prisma.agedReceivableByContact.create({ data: payload });
              logger.info(`AgedReceivable inserted: ${invoiceId}`);
            } else if (
              updatedDateUtc &&
              existing.updatedDateUtc?.toISOString() !==
                updatedDateUtc.toISOString()
            ) {
              await prisma.agedReceivableByContact.updateMany({
                where: {
                  number: rowData[1] || null,
                  invoiceId: invoiceId || null,
                },
              ...
              logger.info(`AgedReceivable updated: ${invoiceId}`);
            }
          } catch (err) {
            logger.error(`AgedReceivable DB error: ${(err as Error).message}`);
          }
        }
      }
    } catch (err) {
      logger.error(
        `AgedReceivables fetch error for ${contactId}: ${(err as Error).message}`,
      );
    }
  }
  logger.info("Aged Receivables By Contact sync complete.");
}
